...
int iterative_deepening(state initial_state){
    int depth = 0;
    while(1){
        visited.clear();
        stack <state> s;
...
int a_star_search(state initial_state){
    visited.clear();
...
void bfs_gen(int lim){
    ...
    q.push(initial_state);
    visited.insert(initial_state.hash_val);
    generated_states[initial_state.depth].push_back(initial_state);
    while(!q.empty()){
        state u = q.front();
        q.pop();
        for(auto v : u.successors()){
            if(visited.find(v.hash_val) == visited.end() && v.depth <= lim){
                q.push(v);
                visited.insert(v.hash_val);
                generated_states[v.depth].push_back(v);
            }
        }
    }
    for(int i = 0; i < lim + 1; i++){
        random_shuffle(generated_states[i].begin(), generated_states[i].end());
        while(generated_states[i].size() > 20) generated_states[i].pop_back();
    }
}
...
int main(){
    ...
    bfs_gen(20);
    cout << "Using bfs," << endl;
    for(int true_step = 0; true_step < 21; true_step++){
        int avg_steps = 0;
        for(auto initial_state : generated_states[true_step]){
            initial_state.depth = 0;
            avg_steps += bfs(initial_state);
        }
        avg_steps /= generated_states[true_step].size();
        cout << true_step << ' ' << avg_steps << endl;
    }


    cout << "Using iterative_deepening," << endl;
    for(int true_step = 0; true_step < 21; true_step++){
        int avg_steps = 0;
        for(auto initial_state : generated_states[true_step]){
            initial_state.depth = 0;
            avg_steps += iterative_deepening(initial_state);
        }
        avg_steps /= generated_states[true_step].size();
        cout << true_step << ' ' << avg_steps << endl;
    }

    cout << "Using bfs a_star_search misplaced" << endl;
    for(int true_step = 0; true_step < 21; true_step++){
        int avg_steps = 0;
        for(auto initial_state : generated_states[true_step]){
            ...
            avg_steps += a_star_search(initial_state);
        }
        avg_steps /= generated_states[true_step].size();
        cout << true_step << ' ' << avg_steps << endl;
    }
    cout << "Using bfs a_star_search manhattan" << endl;
    for(int true_step = 0; true_step < 21; true_step++){
        int avg_steps = 0;
        for(auto initial_state : generated_states[true_step]){
